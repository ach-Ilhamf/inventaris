...
class ExportBarangTerima implements FromArray, WithHeadings, WithEvents
{
    ...
    {
        $columnsToSelect = ['barang_habis_terimas.kode_barang', 'barang_pakai_habis.jenis_barang'];

        $allColumns = Schema::getColumnListing('barang_habis_terimas');
        $remainingColumns = array_diff($allColumns, $this->excludeColumns, ['kode_barang']);

        foreach ($remainingColumns as $column) {
            $columnsToSelect[] = "barang_habis_terimas.$column";
        }

        $query = BarangHabisTerima::with('barangpakaihabis')
            ->select($columnsToSelect)
            ->leftJoin('barang_pakai_habis', 'barang_habis_terimas.id_barang', '=', 'barang_pakai_habis.id')
            ->orderBy('barang_habis_terimas.tgl_terima', 'asc');
        ...
        if (isset($this->filters['tgl_terima']) && !empty($this->filters['tgl_terima'])) {
            $query->where('barang_habis_terimas.tgl_terima', 'like', '%' . $this->filters['tgl_terima'] . '%');
        }
